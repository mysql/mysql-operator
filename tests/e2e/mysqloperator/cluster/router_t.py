...
"""
        ...
        self.assertEqual(self.routers_count, len(pods))
        for instance in range(0, self.routers_count):
            self.assertEqual('0', pods[instance]["RESTARTS"])


    def test_1_load_distribution(self):
        def check(port, hosts_expected):
            # checks that we connect to both secondaries at least once
            h = tutil.run_from_operator_pod(f"mysql://root:sakila@{self.cluster_name}.{self.ns}.svc.cluster.local:{port}",
        ...
        hosts_expected = set([f"{self.cluster_name}-0"])
        self.wait(check, (6446, hosts_expected))

        # ensure connecting to 6447 takes us to all secondaries
        hosts_expected = {f"{self.cluster_name}-{instance}" for instance in range(1, self.cluster_size)}
    ...
    def test_2_stopped_members(self):
        ...
        def check(secondaries_seen):
            # checks that we connect to both secondaries at least once
            h = tutil.run_from_operator_pod(f"mysql://root:sakila@{self.cluster_name}.{self.ns}.svc.cluster.local:6447",
                "print(session.run_sql('select @@hostname').fetch_one()[0])")
        ...
        h = tutil.run_from_operator_pod(f"root:sakila@{self.cluster_name}.{self.ns}.svc.cluster.local:6446",
                "print(session.run_sql('select @@hostname').fetch_one()[0])")
        self.assertEqual(f"{self.cluster_name}-0", h)

        for _ in range(5):
            h = tutil.run_from_operator_pod(f"root:sakila@{self.cluster_name}.{self.ns}.svc.cluster.local:6447",
                "print(session.run_sql('select @@hostname').fetch_one()[0])")
            self.assertIn(h, [f"{self.cluster_name}-2"])
    ...
    def test_3_rest_api(self):
        with kutil.PortForward(self.ns,self.cluster_name, "router-rest", target_type="service") as port:
    ...
    def test_4_remove_router_metadata(self):
        def list_routers() -> dict:
            router_list = kutil.execp(self.ns, (f"{self.cluster_name}-0", "sidecar"),
        ...
        def scale_routers(new_scale: int):
            ...
            self.wait_routers(f"{self.cluster_name}-router-.*", new_scale)
        ...
        name = kutil.ls_po(self.ns, pattern=f"{self.cluster_name}-router-.*")[0]["NAME"]

        kutil.delete_po(self.ns, name)
        self.wait_routers(f"{self.cluster_name}-router-.*", self.routers_count)

        new_name = kutil.ls_po(self.ns, pattern=f"{self.cluster_name}-router-.*")[0]["NAME"]
        ...
        assert_metadata_matches_running_pods(expected_count=self.routers_count)
...
spec:
  ...
  router:
    instances: {self.routers_count}
    podLabels:
      router-label1: "myc-router-label1-value"
      router-label2: "myc-router-label2-value"
    podAnnotations:
      router.myc.example.com/ann1: "rtr-ann1-value"
      router.myc.example.com/ann2: "rtr-ann2-value"
      router.myc.example.com/ann3: "rtr-ann3-value"
  secretName: {self.cluster_secret_name}
  tlsUseSelfSigned: true
"""
    ...
    def test_01_check_initial_ic(self):
        ...
        self.assertEqual(labels['router-label1'], 'myc-router-label1-value')
        self.assertEqual(labels['router-label2'], 'myc-router-label2-value')

        annotations = ic["spec"]["router"]["podAnnotations"]
        self.assertTrue('router.myc.example.com/ann1' in annotations)
        self.assertTrue('router.myc.example.com/ann2' in annotations)
        self.assertTrue('router.myc.example.com/ann3' in annotations)
        self.assertEqual(annotations['router.myc.example.com/ann1'], 'rtr-ann1-value')
        self.assertEqual(annotations['router.myc.example.com/ann2'], 'rtr-ann2-value')
        self.assertEqual(annotations['router.myc.example.com/ann3'], 'rtr-ann3-value')

    def test_03_check_initial_deploy(self):
        ### DEPLOY
        deploy = kutil.get_deploy(self.ns, f"{self.cluster_name}-router")
        ...
        patch = [
            {
                ...
                "value": {
                    "router-label1" : "myc-router-label11-value",
                    "router-label222": "myc-router-label222-value",
                }
            },
            {
                "op":"replace",
                ...
                "value": {
                    "router.myc.example.com/ann1": "rtr-ann111-value",
                    "router.myc.example.com/ann2": "rtr-ann222-value",
                    "router.myc.example.com/ann333": "rtr-ann333-value",
                }
            },
        ]
    ...
    def test_07_check_ic(self):
        labels = kutil.get_ic(self.ns, self.cluster_name)["spec"]["router"]["podLabels"]

        self.assertTrue('router-label1' in labels)
        self.assertTrue('router-label222' in labels)
        self.assertFalse('router-label2' in labels)
        self.assertEqual(labels['router-label1'], 'myc-router-label11-value')
        self.assertEqual(labels['router-label222'], 'myc-router-label222-value')

        annotations = kutil.get_ic(self.ns, self.cluster_name)["spec"]["router"]["podAnnotations"]
        self.assertTrue('router.myc.example.com/ann1' in annotations)
        self.assertTrue('router.myc.example.com/ann2' in annotations)
        self.assertTrue('router.myc.example.com/ann333' in annotations)
        self.assertFalse('router.myc.example.com/ann3' in annotations)
        self.assertEqual(annotations['router.myc.example.com/ann1'], 'rtr-ann111-value')
        self.assertEqual(annotations['router.myc.example.com/ann2'], 'rtr-ann222-value')
        self.assertEqual(annotations['router.myc.example.com/ann333'], 'rtr-ann333-value')
...
"""
    ...
    def test_2_update_bootstrap_config(self):
        ...
        new_pods = kutil.ls_po(self.ns, pattern=f"{self.cluster_name}-router-.*")
    ...
    def test_4_initial_routing_options(self):
        for router_instance in range(0, self.routers_count):
            routing_options = get_routing_options(self.ns, f"{self.cluster_name}-{router_instance}")
