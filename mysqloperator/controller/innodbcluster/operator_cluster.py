...
def on_innodbcluster_create(name, namespace, body, logger, **kwargs):
    ...
    if not cluster.get_create_time():
        ...
        if not cluster.get_router_replica_set():
            if icspec.routers > 0:
                router_replicaset = router_objects.prepare_router_replica_set(icspec)
                kopf.adopt(router_replicaset)
                api_apps.create_namespaced_replica_set(namespace=namespace, body=router_replicaset)
...
def on_innodbcluster_field_instances(old, new, body, logger, **kwargs):
    cluster = InnoDBCluster(body)
    ...
    sts = cluster.get_stateful_set()
    if sts and old != new:
        logger.info(f"Updating InnoDB Cluster StatefulSet.replicas from {old} to {new}")
...
def on_innodbcluster_field_image(old, new, body, logger, **kwargs):
    ...
    sts = cluster.get_stateful_set()
    if sts and old != new:
        logger.info(f"Updating MySQL image for InnoDB Cluster StatefulSet pod template from {old} to {new}")
        cluster.parsed_spec.validate(logger)
...
def on_innodbcluster_field_routers(old, new, body, logger, **kwargs):
    cluster = InnoDBCluster(body)
    ...
    while True:
        try:
            ...
            if pod.phase != "Running" or pod.deleting or not member_info:
                logger.debug(f"ignored pod event: pod={pod.name} containers_ready={ready} deleting={pod.deleting} phase={pod.phase} member_info={member_info}")
                return
            ...
            with ClusterMutex(cluster, pod):
                ...
                if ready and event == "mysql-restarted":
                    ...
                    g_ephemeral_pod_state.set(pod, "mysql-restarts", mysql_restarts)

                # Check if we should refresh the cluster status
                status = cluster_ctl.probe_status_if_needed(pod, logger)
                if status == diagnose.DIAG_CLUSTER_UNKNOWN:
