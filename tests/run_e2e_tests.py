...
if __name__ == '__main__':
    ...
    for arg in sys.argv[2:]:
        if arg.startswith("--env="):
            ...
            g_ts_cfg.s3_skip = True
        elif arg.startswith("--s3-bucket="):
            g_ts_cfg.s3_bucket_name = arg.partition("=")[-1]
        elif arg.startswith("--s3-endpoint="):
            g_ts_cfg.s3_endpoint = arg.partition("=")[-1]
        elif arg.startswith("--s3-profile="):
            g_ts_cfg.s3_profile = arg.partition("=")[-1]
        elif arg.startswith("--s3-config-file="):
