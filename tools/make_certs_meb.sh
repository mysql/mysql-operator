...
MEB_RDN_CN_DEFAULT="backupclient"
KEY_BITS=3072

function usage() {
  echo "Usage:"
  echo "  $0 --interactive"
  echo "  $0 OUT_DIR NAMESPACE CLUSTER_NAME [DOMAIN] [VALIDITY_DAYS] [CA_CN] [MEB_CN]"
}
...
if [[ "${1:-}" == "--interactive" ]]; then
  ...
  out_dir=$(prompt_default "Output directory" "./out/meb/$cluster")
  ...
  meb_cn=$(prompt_default "MEB client certificate common name" "$MEB_RDN_CN_DEFAULT")
else
  if [[ $# -lt 3 || $# -gt 7 ]]; then
    usage >&2
    exit 1
...
validate_subject_value "MEB_CN" "$meb_cn"
...
make_cert "$out_dir" "$out_dir/meb-req.conf" "$out_dir/meb/tls.key" "$out_dir/meb/tls.crt" 03 "$days"
...
echo "MEB TLS secret files: $out_dir/meb/tls.crt $out_dir/meb/tls.key"
